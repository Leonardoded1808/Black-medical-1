...
import type { Client, Opportunity, Salesperson, Interaction, User } from '../types';
...
const Clients: React.FC<ClientsProps> = ({ 
    ...
    const [selectedClientForInteractions, setSelectedClientForInteractions] = useState<Client | null>(null);
    ...
    const isDuplicate = (phone: string) => {
        ...
        return duplicatePhoneNumbers.has(cleanPhone);
    };
    ...
    const handleOpenInteractions = (client: Client) => {
        setSelectedClientForInteractions(client);
        setIsInteractionsModalOpen(true);
    };

    const getClientInteractions = (client: Client) => {
        const clientOpportunities = opportunities.filter(o => o.clientId === client.id);
        const clientOpportunityIds = clientOpportunities.map(o => o.id);
        const clientLeadIds = clientOpportunities.map(o => o.originalLeadId).filter((id): id is string => !!id);

        return interactions.filter(i => 
            i.clientId === client.id ||
            (i.opportunityId && clientOpportunityIds.includes(i.opportunityId)) ||
            (i.leadId && clientLeadIds.includes(i.leadId))
    ...
    return (
        <>
                                    ...
                                    {sortedActiveClients.map(client => {
                                        ...
                                        return (
                                            <React.Fragment key={client.id}>
                                                <tr id={client.id} className={`hover:bg-slate-700/50 transition-colors ${isDuplicate(client.phone) ? 'bg-red-900/10 border-l-4 border-red-500' : ''}`}>
                                                    ...
                                                    <td className="p-4">
                                                        <div className="flex items-center space-x-2">
                                                            <button onClick={() => handleOpenInteractions(client)} className="p-2 text-blue-400 hover:text-blue-300 rounded-md hover:bg-slate-700" title="Ver Interacciones"><ChatBubbleLeftRightIcon className="h-4 w-4" /></button>
                ...
                <div className="md:hidden space-y-4">
                     {sortedActiveClients.length === 0 ? (
                        ...
                        sortedActiveClients.map(client => {
                            const isExpanded = expandedClientId === client.id;
                            return (
                                 <div key={client.id} id={client.id} className={`bg-slate-800 rounded-lg shadow-lg ${isDuplicate(client.phone) ? 'border-2 border-red-500' : ''}`}>
                                    <div className="p-4">
                                        <div className="flex justify-between items-start gap-4">
                                            ...
                                            <div className="flex items-center">
                                                <button onClick={() => handleOpenInteractions(client)} className="p-2 text-blue-400"><ChatBubbleLeftRightIcon className="h-4 w-4" /></button>
            ...
            {isInteractionsModalOpen && selectedClientForInteractions && (
                <InteractionsModal
                    isOpen={isInteractionsModalOpen}
                    onClose={() => setIsInteractionsModalOpen(false)}
                    title={`Interacciones con ${selectedClientForInteractions.name}`}
                    entityId={selectedClientForInteractions.id}
                    entityType="client"
                    interactions={getClientInteractions(selectedClientForInteractions)}
